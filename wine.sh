...
if [[ -n "$GH_REPO" && -n "$GH_TOKEN" ]]; then
	HAVE_WINE_VERSION=$([ "$(curl -s -w '%{http_code}' -o /dev/null "https://x-access-token:${GH_TOKEN}@api.github.com/repos/${GH_REPO}/git/refs/tags/wine-${WINE_VERSION}")" -ne 404 ] && echo "y" || echo "")
	HAVE_STAGING_VERSION=$([ "$(curl -s -w '%{http_code}' -o /dev/null "https://x-access-token:${GH_TOKEN}@api.github.com/repos/${GH_REPO}/git/refs/tags/wine-${STAGING_VERSION}-staging")" -ne 404 ] && echo "y" || echo "")
fi
...
if [ $(echo "$WINE_VERSION > $STAGING_VERSION" | bc -l) -eq 0 ] && [[ -z "$HAVE_WINE_VERSION" || -z "$HAVE_STAGING_VERSION" ]]; then
	...
	git clone -b "wine-${WINE_VERSION}" 'https://gitlab.winehq.org/wine/wine.git' || exit 0
	pushd wine
	git -c advice.detachedHead=false checkout "wine-${WINE_VERSION}"
	...
	git clone -b "v${STAGING_VERSION}" 'https://gitlab.winehq.org/wine/wine-staging.git' || exit 0
	...
	if [ -z "$HAVE_WINE_VERSION" ]; then
 		pushd wine
		[[ -n "$GH_REPO" && -n "$GH_TOKEN" ]] && git push origin "wine-${WINE_VERSION}"
		popd

 		pushd tkg-wine
		git commit -a -m 'TKG...'
		git tag "wine-${WINE_VERSION}-tkg"
		[[ -n "$GH_REPO" && -n "$GH_TOKEN" ]] && git push origin "wine-${WINE_VERSION}-tkg"
		popd
	fi

	pushd staging-wine
	git commit -a -m 'Staging...'
	git tag "wine-${STAGING_VERSION}-staging"
	[[ -n "$GH_REPO" && -n "$GH_TOKEN" ]] && git push origin "wine-${STAGING_VERSION}-staging"
	popd

	pushd tkg-staging-wine
	git commit -a -m 'Staging TKG...'
	git tag "wine-${STAGING_VERSION}-staging-tkg"
	[[ -n "$GH_REPO" && -n "$GH_TOKEN" ]] && git push origin "wine-${STAGING_VERSION}-staging-tkg"
	popd
fi
